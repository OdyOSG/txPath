...
#' @include utils.R stratafication.R
...
create_treatment_history <- function(connectionDetails,
  ...
  #create stratas
  strataNm <- purrr::map_chr(analysisSettings$strata, ~.x$varName)
  strata <- purrr::map(analysisSettings$strata, ~eval(.x$call)) %>%
    purrr::set_names(strataNm) %>%
    dplyr::bind_cols()
